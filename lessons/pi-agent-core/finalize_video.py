...
def main():
    ...
    for item, scene in zip(timeline, project['scenes']):
        ...
        assert item['scene']['boardAnimation'] == scene['boardAnimation']
    ...
    embedded = subprocess.run([binary,'-v','error','-i',str(final),'-f','ffmetadata','-'],
                              capture_output=True,encoding='utf-8',check=True).stdout
    assert embedded.count('[CHAPTER]') == len(chapters)
    for c in chapters:
        assert 'title='+escape(c['name']) in embedded
    (out/'embedded-chapters.ffmeta').write_text(embedded,encoding='utf-8')
    ...
    report = {'duration_seconds':duration,'container_duration_seconds':stream['duration'],
              ...
              'full_av_decode':'passed','caption_and_scene_timeline':'passed','embedded_chapters':'passed',
              'board_animation_snapshot':'passed',
