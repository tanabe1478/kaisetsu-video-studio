...
新しいコンテンツは、画面上部の「制作ブリーフ」でテーマ・目標時間・想定視聴者・構成などを入力し、作成した依頼文をChatGPTに渡します。最終成果物は既定で音声付き本編動画と台本JSONです。台本のみ、または先に構成案を確認する進め方も選べます。
制作フローは[content-production Skill](.agents/skills/content-production/SKILL.md)に記録しています。制作条件が未確認なら台本・動画制作の前に入力を尋ね、回答済みの条件は引き継ぎます。
...
本編生成の最後に映像・音声の全編デコード、フレーム数、音声長、字幕時刻、無音でないことを確認し、成功時だけ `delivery.json` を保存します。再検証は `python verify_video.py output`。動画制作の完了時には、本編MP4へのリンクと完成尺を渡します。発音や図の意味のレビューは、この自動検証とは別です。
