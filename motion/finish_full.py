...
(out/'index.html').write_text('''<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>プロンプトキャッシュとPi — 完全版</title><style>body{margin:28px auto;max-width:1200px;padding:0 24px;background:#f6f3e9;color:#173d35;font:17px system-ui}video{width:100%;border-radius:14px;background:#173d35}button{padding:12px;margin:5px;border:1px solid #819c8c;border-radius:8px;background:white;color:#173d35;cursor:pointer;text-align:left}h1{font-size:28px}.chapters{display:grid;grid-template-columns:1fr 1fr}small{color:#50675d}</style><h1>プロンプトキャッシュとPiの設計 — 完全版</h1><p>20分2.75秒 · 30場面 · 2人の掛け合い · Remotion / SVG</p><video controls preload="metadata" src="demo.mp4"></video><p id="time">動画を読み込み中…</p><div class="chapters">'''+buttons+'''</div><p><a href="demo.mp4" download>完全版MP4を保存</a> · <a href="subtitles.srt" download>字幕SRT</a></p><small>VOICEVOX：四国めたん・ずんだもん。図は模式図。出典・固定コミットは台本に保持。</small><script>const v=document.querySelector('video');fetch('demo.mp4').then(r=>r.blob()).then(b=>{v.src=URL.createObjectURL(b);document.querySelector('#time').textContent='再生できます'}).catch(e=>document.querySelector('#time').textContent=e.message);document.querySelectorAll('[data-time]').forEach(b=>b.onclick=()=>{v.pause();v.currentTime=Number(b.dataset.time)});v.ontimeupdate=()=>document.querySelector('#time').textContent=Math.floor(v.currentTime/60)+':'+String(Math.floor(v.currentTime)%60).padStart(2,'0')+' / 20:03';</script></html>''')
