...
async function api(url,data,retry=true){const r=await fetch(url,{method:data?'POST':'GET',headers:data?{'Content-Type':'application/json','X-Editor-Token':token}:{},body:data?JSON.stringify(data):undefined});const result=await r.json();if(r.status===403&&data&&retry){const boot=await api('/api/bootstrap');token=boot.token;return api(url,data,false);}if(!r.ok){if(r.status===409)conflict=true;throw new Error(result.error||'保存できませんでした。');}return result;}
