...
let boardTimer=null,boardTicket=0,boardLineId=null;
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const id=()=>crypto.randomUUID().replaceAll('-','');
const clone=x=>structuredClone(x);
let durationTimer=null,durationResult=null,briefDirty=false;
...
async function save(){
 if($('#editor [data-board="nodes"]:invalid'))throw Error('黒板の入力エラーを直してから保存してください。');
...
function renderStats(){scheduleDuration();const n=allScenes().reduce((a,s)=>a+s.lines.reduce((b,l)=>b+l.text.length,0),0);$('#stats').textContent=p?`${p.chapters.length}章 / ${allScenes().length}場面 / ${n.toLocaleString()}文字`:'章・場面を並べて考える';}
...
$('#export').onclick=async()=>{try{clearError();await save();const result=await api('/api/export',{project:p});$('#handoff-text').value=`次のフォルダーに、編集済みの台本とフィードバックがあります。\n${result.directory}\n\nscript.jsonとfeedback.mdを読み、私が変更した文言・構成を尊重して、未対応のフィードバックを反映した改訂案を作ってください。元ファイルは残してください。`;if(p.meta.targetMinutes)$('#handoff-text').value+=`\n\n完成尺の目標は${p.meta.targetMinutes}分です。説明の量と構成を調整し、章ごとの時間配分も示してください。極端な話速変更や無音で尺を合わせないでください。指定が両立しない場合は省略・分割案を示してください。`;$('#export-dialog').showModal();}catch(e){error(e);}};
...
document.addEventListener('click',e=>{
 ...
 switch(d.action){
 case 'duration-request':if(!p.meta.targetMinutes){toast('まず目標時間を入力してください');break;}$('#export').click();break;
...
document.addEventListener('input',e=>{
 ...
 if(e.target.type==='number'&&(value===''||!Number.isFinite(Number(value))||!e.target.checkValidity()))return;
 mutate(()=>{
  if(field.startsWith('project-')){const k=field.slice(8);p.meta[k]=['speed','targetMinutes'].includes(k)?Number(value):value;return;}
...
document.addEventListener('change',e=>{
 ...
 else {mutate(()=>current().s.data[field.slice(6)]=e.target.value);scheduleBoardPreview();const panel=e.target.closest('.settings');if(panel){const sceneSprite=panel.querySelector('.sprite-preview');if(sceneSprite)sceneSprite.outerHTML=spritePreview(current().s.data.expression,current().s.data.pose);for(const card of document.querySelectorAll('.line-card')){const line=current().s.lines.find(l=>l.id===card.dataset.line),d=line.direction||{};const sprite=card.querySelector('.sprite-preview');if(sprite)sprite.outerHTML=spritePreview(d.expression??current().s.data.expression,d.pose??current().s.data.pose);}return;}}
 renderAll();
});
...
window.addEventListener('beforeunload',e=>{if(generation!==savedGeneration||briefDirty||$('#editor [data-board="nodes"]:invalid')){e.preventDefault();e.returnValue='';}});
...
function deliveryControls(line,scene){const d=line.direction||{};return `<details class="delivery"><summary>テンポ・立ち絵 ${d.source==='auto'?'· 自動提案':line.direction?'· 手動設定':'· 場面設定を継承'}</summary>${linePerson(line).art==='zundamon'?spritePreview(d.expression??scene.data.expression,d.pose??scene.data.pose):'<p>この話者の立ち絵は黒板プレビューで確認できます。</p>'}<div class="settings-grid"><label>話速<input type="number" min="0.5" max="2" step="0.01" data-delivery="speed" data-line-id="${line.id}" value="${d.speed??p.meta.speed??1}"></label><label>セリフ後の追加の間（秒）<input type="number" min="0" max="10" step="0.05" data-delivery="pause" data-line-id="${line.id}" value="${d.pause??0}"></label><label>表情<select data-delivery="expression" data-line-id="${line.id}">${options([['normal','ふつう'],['happy','笑顔'],['surprised','驚き'],['thinking','考える']],d.expression??scene.data.expression??'normal')}</select></label><label>ポーズ<select data-delivery="pose" data-line-id="${line.id}">${options([['normal','ふつう'],['wave','手を挙げる'],['point','指差し'],['think','考える']],d.pose??scene.data.pose??'normal')}</select></label></div><small>${esc(d.reason||'未指定項目は台本・場面の設定を使います。')} ／変更したセリフは自動提案で上書きしません。</small></details>`;}
function editDelivery(el){if(!p||!current())return;const key=el.dataset.delivery,numeric=['speed','pause'].includes(key);if(numeric&&(el.value===''||!el.checkValidity()))return;const line=current().s.lines.find(l=>l.id===el.dataset.lineId);if(!line)return;mutate(()=>{line.direction={...line.direction,[key]:numeric?Number(el.value):el.value,source:'manual'};delete line.direction.reason;},'delivery:'+line.id+key);el.closest('details').querySelector('summary').textContent='テンポ・立ち絵 · 手動設定';scheduleBoardPreview(line.id);const preview=el.closest('details').querySelector('.sprite-preview');if(preview)preview.outerHTML=spritePreview(line.direction.expression??current().s.data.expression,line.direction.pose??current().s.data.pose);}
...
function repositorySources(scene){const repo=p.meta.repository;if(!repo||!/^https:\/\/github\.com\/[\w.-]+\/[\w.-]+\/?$/.test(repo.url||'')||! /^[a-f0-9]{40}$/.test(repo.commit||''))return '';const base=repo.url.replace(/\/$/,'');const links=(scene.data.sources||[]).filter(x=>typeof x.path==='string'&&Number.isInteger(x.startLine)&&x.startLine>0).map(x=>{const path=x.path.split('/').map(encodeURIComponent).join('/');const end=Number.isInteger(x.endLine)&&x.endLine>=x.startLine?'-L'+x.endLine:'';return `<li><a href="${esc(base)}/blob/${repo.commit}/${path}#L${x.startLine}${end}" target="_blank" rel="noopener noreferrer">${esc(x.path)}:${x.startLine}</a> — ${esc(x.note||'参照箇所')}</li>`;}).join('');return `<details class="repository-sources"><summary>リポジトリの出典 · ${repo.commit.slice(0,7)}</summary><p><a href="${esc(base)}/tree/${repo.commit}" target="_blank" rel="noopener noreferrer">${esc(base.split('/').slice(-2).join('/'))}</a> ／ ${esc(repo.kind||'ソース調査')}</p><ul>${links}</ul><small>固定したコミットの参照です。教材の説明用コードと原文を区別してください。</small></details>`;}
$('#repository-request').onclick=()=>{const url=$('#repository-url').value.trim().replace(/\/$/,'').replace(/\.git$/,'');if(!/^https:\/\/github\.com\/[\w.-]+\/[\w.-]+$/.test(url)){error(Error('https://github.com/owner/repo の形式で入力してください。'));return;}clearError();$('#repository-prompt').value=`次のリポジトリを題材に解説台本を作ってください。\n${url}\n\nまずcontent-production Skillに従い、制作ブリーフが未確認なら私に入力を依頼してください。回答後にdocs/REPOSITORY_LESSONS.mdと台本表記Skillに従い、コミットを固定して調査してください。構成・主要処理・データの流れ・制約を説明し、各場面に出典を付けて台本ノートへ読み込めるJSONを作ってください。記事の分析と実装で確認した事実を区別し、既存の編集済み台本は残してください。`;$('#repository-prompt').hidden=false;$('#repository-copy').hidden=false;};
$('#repository-copy').onclick=async()=>{try{await navigator.clipboard.writeText($('#repository-prompt').value);toast('依頼文をコピーしました。この会話に貼り付けてください');}catch{$('#repository-prompt').select();toast('Ctrl+Cでコピーしてください');}};
...
$('#open-brief').onclick=async()=>{try{await briefSaving;const b=await api('/api/brief');$('#brief-length').innerHTML=lengthControls('brief',b.targetMinutes??10,b.targetLength);document.querySelectorAll('[data-brief]').forEach(el=>{if(el.type==='checkbox')el.checked=b[el.dataset.brief]??false;else el.value=b[el.dataset.brief]??(el.dataset.brief==='deliverable'?'video':el.type==='number'?10:'');});$('#brief-result').hidden=true;$('#brief-error').hidden=true;$('#brief-dialog').showModal();}catch(e){error(e);}};
...
function people(){return p.meta.characters?.length?p.meta.characters:[{id:'host',name:p.meta.speaker||'ずんだもん',speaker:p.meta.speaker||'ずんだもん',style:p.meta.style||'ノーマル',art:'zundamon',color:'#bce88a'}];}
function linePerson(line){return people().find(c=>c.id===line.character)||people()[0];}
function ensureCast(){if(!p.meta.characters?.length)p.meta.characters=clone(people());}
function castControls(){return `<section class="duration-panel"><h3>解説形式・話者</h3><label>形式<select data-presentation="mode">${options([['solo','1人解説'],['dialogue','2人の掛け合い']],p.meta.presentationMode||'solo')}</select></label><button data-cast-preset="yukkuri">霊夢・魔理沙に設定</button><small>霊夢・魔理沙の画像＋VOICEVOXの代替音声です。ゆっくり音声を使う場合は各セリフに音声ファイルを指定できます。</small>${people().map((c,i)=>`<details class="cast-member"><summary>${i+1}人目：${esc(c.name)}</summary><div class="settings-grid">${[['name','表示名'],['speaker','VOICEVOX話者'],['style','音声スタイル'],['art','画像：zundamon / avatar / assets内のパス'],['mouthArt','口開き画像のパス（任意）'],['credit','素材クレジット']].map(([k,label])=>`<label>${label}<input data-character-index="${i}" data-character-key="${k}" value="${esc(c[k]||'')}"></label>`).join('')}<label>字幕色<input type="color" data-character-index="${i}" data-character-key="color" value="${esc(c.color||'#bce88a')}"></label></div></details>`).join('')}</section>`;}
function dialogueLineControls(line,scene){return `<div class="settings-grid dialogue-controls"><label>このセリフの話者<select data-dialogue="character" data-line-id="${line.id}">${options(people().map(c=>[c.id,c.name]),line.character||people()[0].id)}</select></label><label>音声ファイル（任意・assets内）<input data-dialogue="audio" data-line-id="${line.id}" value="${esc(line.audio||'')}" placeholder="assets/audio/reimu-01.wav"></label>${scene.data.board&&!scene.data.boardAnimation?`<label>黒板に表示する項目数<select data-dialogue="boardStep" data-line-id="${line.id}">${options([[0,'全部'],...scene.data.board.nodes.map((_,i)=>[i+1,`${i+1}項目まで`])].map(([v,t])=>[String(v),t]),String(line.boardStep||0))}</select></label><label>黒板の強調<select data-dialogue="boardFocus" data-line-id="${line.id}">${options([['0','なし'],...scene.data.board.nodes.map((n,i)=>[String(i+1),`${i+1}：${n.label}`])],String(line.boardFocus||0))}</select></label>`:''}<button data-board-line="${line.id}">このセリフの画面を見る</button></div>`;}
function boardControls(scene){if(scene.data.boardAnimation){const a=scene.data.boardAnimation;return `<section class="board-panel"><h3>AIが設計した黒板アニメーション</h3><p>${esc(a.intent)}</p><p>動きは「場面プレビュー」で確認できます。項目の配置や秒数の入力は不要です。気になる点を編集メモに残し、「AIに渡す」で再調整を依頼してください。</p><label>プレビューするセリフ<select id="board-preview-line">${options(scene.lines.map((l,i)=>[l.id,`${i+1}：${linePerson(l).name}「${l.text.slice(0,24)}」`]),scene.lines.some(l=>l.id===boardLineId)?boardLineId:scene.lines[0]?.id)}</select></label><p id="board-preview-status" role="status"></p><img id="board-preview-image" alt="動画と共通の黒板・話者プレビュー" hidden><small>静止画は選択セリフの75%時点。セリフの変更・並べ替え後はAIによる動きの再調整が必要です。</small></section>`;}const b=scene.data.board;return `<section class="board-panel"><h3>黒板の図解</h3><label>図の種類<select data-board="layout">${options([['off','使わない'],['flow','流れ・手順（→）'],['comparison','比較（左右のカード）'],['relation','関係（↔）']],b?.layout||'off')}</select></label>${b?`<label>項目（1行に「見出し | 説明」、最大4項目）<textarea data-board="nodes" rows="4">${esc(b.nodes.map(n=>n.label+(n.detail?' | '+n.detail:'')).join('\n'))}</textarea></label><small>セリフごとに表示する項目数・強調を選べます。図解はコード例より優先して表示します。</small>`:''}<label>プレビューするセリフ<select id="board-preview-line">${options(scene.lines.map((l,i)=>[l.id,`${i+1}：${linePerson(l).name}「${l.text.slice(0,24)}」`]),scene.lines.some(l=>l.id===boardLineId)?boardLineId:scene.lines[0]?.id)}</select></label><p id="board-preview-status" role="status"></p><img id="board-preview-image" alt="動画と共通の黒板・話者プレビュー" hidden><small>静止画で配置を確認できます。口パク・音声は「場面プレビュー」で確認してください。</small></section>`;}
function scheduleBoardPreview(lineId){clearTimeout(boardTimer);const ticket=++boardTicket;if(!current()||!$('#board-preview-image'))return;if(lineId){boardLineId=lineId;$('#board-preview-line').value=lineId;}boardTimer=setTimeout(async()=>{const img=$('#board-preview-image');if(!img||!current())return;$('#board-preview-status').textContent='画面を更新中…';try{const r=await api('/api/board-preview',{project:clone(p),sceneId:current().s.id,lineId:$('#board-preview-line').value});if(ticket!==boardTicket)return;img.src=r.image;img.hidden=false;$('#board-preview-status').textContent='プレビュー更新済み';}catch(e){if(ticket===boardTicket){img.hidden=true;$('#board-preview-status').textContent=e.message;}}},350);}
document.addEventListener('click',e=>{const preset=e.target.dataset.castPreset;if(preset){mutate(()=>{const old=people();p.meta.presentationMode='dialogue';p.meta.characters=[{id:old[0]?.id||'host',name:'霊夢',speaker:'四国めたん',style:'ノーマル',art:'kitsune:reimu',color:'#ffc7cc',credit:'きつね（仮）／東方Project二次創作'},{id:old[1]?.id||'guest',name:'魔理沙',speaker:'ずんだもん',style:'ノーマル',art:'kitsune:marisa',color:'#ffe08a',credit:'きつね（仮）／東方Project二次創作'}];});renderAll();}if(e.target.dataset.boardLine){scheduleBoardPreview(e.target.dataset.boardLine);$('#board-preview-image').scrollIntoView({block:'center',behavior:'smooth'});}});
document.addEventListener('change',e=>{
 const el=e.target;if(el.id==='board-preview-line'){boardLineId=el.value;scheduleBoardPreview();return;}
 if(el.dataset.presentation){mutate(()=>{ensureCast();p.meta.presentationMode=el.value;if(el.value==='dialogue'&&p.meta.characters.length===1)p.meta.characters.push({id:p.meta.characters[0].id==='guest'?'guest2':'guest',name:'聞き手',speaker:'四国めたん',style:'ノーマル',art:'avatar',color:'#ffdb9c'});});renderAll();}
 if(el.dataset.board==='layout'){mutate(()=>{const s=current().s;if(el.value==='off')delete s.data.board;else s.data.board={layout:el.value,nodes:s.data.board?.nodes||[{label:'最初の項目',detail:''},{label:'次の項目',detail:''}]};});renderAll();}
 if(el.dataset.dialogue&&el.tagName==='SELECT'){mutate(()=>{const l=current().s.lines.find(l=>l.id===el.dataset.lineId);l[el.dataset.dialogue]=el.dataset.dialogue==='character'?el.value:Number(el.value);});const card=el.closest('.line-card');card.querySelector('.speaker').textContent=linePerson(current().s.lines.find(l=>l.id===el.dataset.lineId)).name;if(el.dataset.dialogue==='character')renderEditor();scheduleBoardPreview(el.dataset.lineId);}
});
document.addEventListener('input',e=>{
 const el=e.target;
 if(el.dataset.characterKey){mutate(()=>{ensureCast();p.meta.characters[Number(el.dataset.characterIndex)][el.dataset.characterKey]=el.value;},'cast:'+el.dataset.characterIndex+el.dataset.characterKey);}
 if(el.dataset.dialogue==='audio'){mutate(()=>{const l=current().s.lines.find(l=>l.id===el.dataset.lineId);if(el.value.trim())l.audio=el.value.trim();else delete l.audio;},'audio:'+el.dataset.lineId);}
 if(el.dataset.board==='nodes'){
  const nodes=el.value.split('\n').filter(x=>x.trim()).map(x=>{const [label,...detail]=x.split('|');return {label:label.trim(),detail:detail.join('|').trim()};});
  if(!nodes.length||nodes.length>4||nodes.some(n=>!n.label||n.label.length>24||n.detail.length>60)){$('#board-preview-status').textContent='1〜4項目、見出し24文字・説明60文字以内で入力してください。';el.setCustomValidity('黒板の項目を確認してください');clearTimeout(boardTimer);boardTicket++;$('#board-preview-image').hidden=true;return;}
  el.setCustomValidity('');mutate(()=>current().s.data.board.nodes=nodes,'board:'+current().s.id);for(const l of current().s.lines){for(const key of ['boardStep','boardFocus']){const select=document.querySelector(`[data-dialogue="${key}"][data-line-id="${l.id}"]`);if(select)select.innerHTML=options([['0',key==='boardStep'?'全部':'なし'],...nodes.map((n,i)=>[String(i+1),key==='boardStep'?`${i+1}項目まで`:`${i+1}：${n.label}`])],String(l[key]||0));}}scheduleBoardPreview();
 }
 if(el.dataset.field?.startsWith('scene-')||el.dataset.field==='line-text')scheduleBoardPreview(el.dataset.lineId);
});
